...
def predict():
    ...
    marked_image = mark_bottom_points(binary_thresh, binary_thresh)  # Change here
    ...
    # First model prediction (NORMAL vs AMD)
    prob1 = model1.predict_proba(features)[0]  # Get probabilities
    labels1 = model1.classes_
    pred1 = labels1[np.argmax(prob1)]  # Use max probability for prediction

    normal_vs_amd_probs = {labels1[i]: prob1[i] for i in range(len(labels1))}

    # Second model prediction (DRUSEN vs CNV)
    prob2 = model2.predict_proba(features)[0]  # Get probabilities
    labels2 = model2.classes_
    pred2 = labels2[np.argmax(prob2)]  # Use max probability for prediction
